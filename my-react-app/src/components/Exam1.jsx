...
class Exam1 extends Component {
	...
	constructor(props) {
		...
		// Exam1 컴포넌트에서 사용할 상태를 정의(state)
		// this.state : 클래스형 컴포넌트에서 컴포넌트의 상태(state) 객체를 의미함
		this.state = {
			count: 0	// state 중 count 상태 값을 0으로 초기화
		};
	...
	componentDidMount() {
		console.log("componentDidMount : 마운트 완료(생성됨)");
	}
	...
	// 렌더링 함수 . JSX 를 반환하여 화면에 UI를 그린다(렌더링)
	render() {
		return (
			<div>
				...
				<h2>
					부모로부터 전달받은 값 : {this.props.ddiyong} , {this.props.test}
				</h2>
			</div>
		);
	}
